...
Hi! This is my first blog post on my first website created with the R `blogdown` package and I am going to document my journey and give a few tips in this post.
...
You are going to generate a static website. It means a bunch of pages with content, formatting and style. The `blogdown` package objective is to make Rmarkdown compatible with website templates from [Hugo](https://gohugo.io/). Hugo templates take care of the style and formatting so that you can focus on Rmarkdown content. The only thing you need to do is to choose a [theme](https://themes.gohugo.io/). There is a huge list of themes to choose from but you will be better with a well maintained theme and [this link](https://pfht.netlify.app/post/top-starred/) sorts them by decreasing number of stars on github to help you choose.  

However, Hugo themes might not be all compatible with blogdown. If you are just starting like me, you would be better with a theme developed or recommended by blogdown authors or widely adopted by the community.
...
Both Xmin and Lithium themes have things I want and others I don't:
...
Overall it seemed easier to add content to Lithium home page than add a navbar logo to Xmin so I went with Lithium. I also started another site in another R project with Xmin for testing new features.
...
- Create a new site. You have 3 options:  
    + from a R project based on a Github repo then:  
        `blogdown::new_site(theme = "yihui/hugo-lithium")` 
        (Alison [post](https://www.apreshill.com/blog/2020-12-new-year-new-blogdown/), requires a Github account, R and Rstudio recommended), 
    + from a Rstudio project with:   
        `Rstudio > New Project... > Website using Blogdown`  
        (requires R and Rstudio),
    + directly with:  
    `blogdown::new_site(theme = "yihui/hugo-lithium")`  
    after setting the working directory to your desired location (suitable if you are not using Rstudio).
...
Now is a good time to read sections on content, template and custom layout of the blogdown book (note to myself to keep these links up-to-date).
